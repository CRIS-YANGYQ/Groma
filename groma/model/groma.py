    ...
    def forward(
        ...
        if past_key_values is None:
            with torch.no_grad():
                ...
                # Convert scores_fused to a list of lists, one list per batch
                scores_fused_list = [scores_fused[i].tolist() for i in range(bs)]
                # select regions to encode
                selected_boxes = []
                selected_scores = [] # Added：存储对应的 scores_fused
                ...
                for i in range(bs):
                    ...
                    if len(nms_inds) > 0:
                        ...
                        selected_scores.append(scores[nms_inds])  # Added：存储选中的 scores
                        ...
                        selected_scores.append(scores[max_ind: max_ind + 1])  # Added
            ...
            print('len of pred_boxes:', len(pred_boxes[0]))
            print('len of selected_boxes:', len(selected_boxes[0]))
            vis_outputs = {
                'pred_boxes': selected_boxes,
                'image_features': image_features,
                'region_features': region_features.detach(),
                'scores': selected_scores # Added：存储 scores_fused
            }
        else:
